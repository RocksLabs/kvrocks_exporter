...
func getTestExporterWithOptions(opt Options) *Exporter {
	...
	e, _ := NewKvrocksExporter(addr, opt)
	return e
}
...
func TestIncludeSystemMemoryMetric(t *testing.T) {
	for _, inc := range []bool{false, true} {
		...
		e, _ := NewKvrocksExporter(os.Getenv("TEST_REDIS_URI"), Options{Namespace: "test", InclSystemMetrics: inc})
		r.Register(e)
...
func TestNonExistingHost(t *testing.T) {
	e, _ := NewKvrocksExporter("unix:///tmp/doesnt.exist", Options{Namespace: "test"})
