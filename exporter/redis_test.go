...
func TestHostVariations(t *testing.T) {
	...
	for _, prefix := range []string{"", "redis://", "tcp://", ""} {
		e, _ := NewKvrocksExporter(prefix+host, Options{SkipTLSVerification: true})
		c, err := e.connectToKvrocks()
		if err != nil {
			t.Errorf("connectToKvrocks() err: %s", err)
			continue
		}
...
func TestPasswordInvalid(t *testing.T) {
	...
	e, _ := NewKvrocksExporter(uri, Options{Namespace: "test", Registry: prometheus.NewRegistry()})
	ts := httptest.NewServer(e)
	defer ts.Close()
