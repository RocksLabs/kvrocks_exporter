...
func TestHTTPScrapeMetricsEndpoints(t *testing.T) {
	...
	for _, tst := range []struct {
		...
		t.Run(tst.name, func(t *testing.T) {
			...
			e, _ := NewKvrocksExporter(tst.addr, options)
...
func TestSimultaneousMetricsHttpRequests(t *testing.T) {
	...
	e, _ := NewKvrocksExporter("", Options{Namespace: "test", InclSystemMetrics: false, Registry: prometheus.NewRegistry()})
	ts := httptest.NewServer(e)
	defer ts.Close()
...
func TestHttpHandlers(t *testing.T) {
	...
	e, _ := NewKvrocksExporter(os.Getenv("TEST_PWD_REDIS_URI"), Options{Namespace: "test", Registry: prometheus.NewRegistry()})
	ts := httptest.NewServer(e)
	defer ts.Close()
